...
#define SIMSIMD_TARGET_ARM_SVE 1
    ...
    (defined(USEARCH_DEFINED_LINUX) || defined(USEARCH_DEFINED_APPLE))
...
inline bool str_equals(char const *begin, std::size_t len,
                       char const *other_begin) noexcept {
...
inline expected_gt<metric_kind_t> metric_from_name(char const *name,
  ...
  if (str_equals(name, len, "l2sq") || str_equals(name, len, "euclidean_sq")) {
    ...
    parsed.failed("Unknown distance, choose: l2sq, ip, cos, haversine, "
                  "divergence, jaccard, pearson, hamming, "
                  "tanimoto, sorensen");
  return parsed;
}
 ...
 *          Isn't efficient for small batches, as it recreates the threads on
 * every call.
 */
...
public:
  /**
   *  @param threads_count The number of threads to be used for parallel
   * execution.
   */
  executor_stl_t(std::size_t threads_count = 0) noexcept
      : threads_count_(threads_count ? threads_count
  ...
  /**
   *  @brief Executes a fixed number of tasks using the specified thread-aware
   * function.
   *  @param tasks                 The total number of tasks to be executed.
   *  @param thread_aware_function The thread-aware function to be called for
   * each thread index and task index.
   *  @throws If an exception occurs during execution of the thread-aware
   * function.
   */
  template <typename thread_aware_function_at>
  void fixed(std::size_t tasks,
    ...
    if (threads_count > 1) {
      tasks_per_thread =
          (tasks / threads_count) + ((tasks % threads_count) != 0);
      for (std::size_t thread_idx = 1; thread_idx < threads_count;
           ++thread_idx) {
        threads_pool.emplace_back([=]() {
          for (std::size_t task_idx = thread_idx * tasks_per_thread;
               task_idx < (std::min)(tasks, thread_idx * tasks_per_thread +
                                                tasks_per_thread);
               ++task_idx)
    ...
    for (std::size_t task_idx = 0;
         task_idx < (std::min)(tasks, tasks_per_thread); ++task_idx)
      thread_aware_function(0, task_idx);
  }

  /**
   *  @brief Executes limited number of tasks using the specified thread-aware
   * function.
  ...
  dynamic(std::size_t tasks,
    ...
    for (std::size_t task_idx = 0;
         task_idx < (std::min)(tasks, tasks_per_thread) &&
         !stop.load(std::memory_order_relaxed);
         ++task_idx)
  ...
  /**
   *  @brief Saturates every available thread with the given workload, until
   * they finish.
   *  @param thread_aware_function The thread-aware function to be called for
   * each thread index.
   *  @throws If an exception occurs during execution of the thread-aware
  ...
  template <typename thread_aware_function_at>
  void
  parallel(thread_aware_function_at &&thread_aware_function) noexcept(false) {
    if (threads_count_ == 1)
 ...
 *          Is the preferred implementation, when available, and maximum
...
public:
  ...
  executor_openmp_t(std::size_t threads_count = 0) noexcept {
    omp_set_num_threads(threads_count ? threads_count
                                      : std::thread::hardware_concurrency());
  }
...
#pragma omp parallel for schedule(dynamic, 1)
  ...
  dynamic(std::size_t tasks,
          thread_aware_function_at &&thread_aware_function) noexcept(false) {
    // OpenMP cancellation points are not yet available on most platforms, and
    // require the `OMP_CANCELLATION` environment variable to be set.
...
public:
  ...
  pointer allocate(size_type length) const {
    std::size_t length_bytes = alignment_ak * divide_round_up<alignment_ak>(
...
public:
  ...
  /**
   *  @brief Allocates an @b uninitialized block of memory of the specified
   * size.
...
#if defined(USEARCH_DEFINED_WINDOWS)
    return (byte_t *)(::VirtualAlloc(NULL, count_bytes,
                                     MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
#else
    return (byte_t *)mmap(NULL, count_bytes, PROT_WRITE | PROT_READ,
                          MAP_PRIVATE | MAP_ANONYMOUS, 0, 0);
#endif
  }

  void deallocate(byte_t *page_pointer,
...
/**
 *  @brief  Memory-mapping allocator designed for "alloc many, free at once"
 * usage patterns.
 *          @b Thread-safe, @b except constructors and destructors.
 *
 *  Using this memory allocator won't affect your overall speed much, as that is
 * not the bottleneck. However, it can drastically improve memory usage
 * especially for huge indexes of small vectors.
 */
template <std::size_t alignment_ak = 1> class memory_mapping_allocator_gt {
  ...
  static constexpr std::size_t head_size() {
    ...
    return divide_round_up<alignment_ak>(sizeof(byte_t *) +
                                         sizeof(std::size_t)) *
           alignment_ak;
  }
...
public:
  ...
  operator=(memory_mapping_allocator_gt &&other) noexcept {
  ...
  void reset() noexcept {
    ...
    while (last_arena) {
      ...
      std::memcpy(&last_cap, last_arena + sizeof(byte_t *),
                  sizeof(std::size_t));
   ...
   *  @note This is a no-op copy constructor since the allocator is not
   * copyable.
   */
   ...
   *  @note This is a no-op copy assignment operator since the allocator is not
   * copyable.
   *  @return Reference to the allocator after the assignment.
   */
  memory_mapping_allocator_gt &
  operator=(memory_mapping_allocator_gt const &) noexcept {
    reset();
    return *this;
  }
  ...
  inline byte_t *allocate(std::size_t count_bytes) noexcept {
    std::size_t extended_bytes =
        divide_round_up<alignment_ak>(count_bytes) * alignment_ak;
    std::unique_lock<std::mutex> lock(mutex_);
    if (!last_arena_ || (last_usage_ + extended_bytes >= last_capacity_)) {
      std::size_t new_cap = (std::max)(last_capacity_, ceil2(extended_bytes)) *
                            capacity_multiplier();
  ...
  /**
   *  @brief Returns the amount of memory used by the allocator across all
   * arenas.
  ...
  /**
   *  @brief Returns the amount of remaining memory already reserved but not yet
   * used.
...
/**
 *  @brief  C++11 userspace implementation of an oversimplified
 * `std::shared_mutex`, that assumes rare interleaving of shared and unique
 * locks. It's not fair, but requires only a single 32-bit atomic integer to
 * work.
 */
...
public:
  ...
  /**
   *  @brief Try upgrades the current `lock_shared()` to a unique `lock()`
   * state.
   */
  inline bool try_escalate() noexcept {
    std::int32_t one_read = 1;
    return state_.compare_exchange_weak(one_read, writing_k,
...
template <typename from_scalar_at, typename to_scalar_at> struct cast_gt {
  inline bool operator()(byte_t const *input, std::size_t dim,
                         byte_t *output) const {
    from_scalar_at const *typed_input =
        reinterpret_cast<from_scalar_at const *>(input);
...
template <typename from_scalar_at> struct cast_gt<from_scalar_at, b1x8_t> {
  inline bool operator()(byte_t const *input, std::size_t dim,
    ...
    for (std::size_t i = 0; i != dim; ++i)
      typed_output[i / CHAR_BIT] |=
          bool(typed_input[i]) ? (128 >> (i & (CHAR_BIT - 1))) : 0;
    return true;
  }
};
...
template <typename to_scalar_at> struct cast_gt<b1x8_t, to_scalar_at> {
  inline bool operator()(byte_t const *input, std::size_t dim,
                         byte_t *output) const {
    unsigned char const *typed_input =
        reinterpret_cast<unsigned char const *>(input);
    to_scalar_at *typed_output = reinterpret_cast<to_scalar_at *>(output);
    for (std::size_t i = 0; i != dim; ++i)
      typed_output[i] =
          bool(typed_input[i / CHAR_BIT] & (128 >> (i & (CHAR_BIT - 1))));
    return true;
  }
};
...
public:
  ...
  inline i8_converted_t(f16_t v)
      : int8_(usearch::clamp<std::int8_t>(
            static_cast<std::int8_t>(v * divisor_k), min_k, max_k)) {}
  ...
  static_assert( //
      std::is_unsigned<scalar_t>::value ||
          (std::is_enum<scalar_t>::value &&
           std::is_unsigned<
               typename std::underlying_type<scalar_t>::type>::value),
 ...
 *          Often used in chemistry and biology to compare molecular
 * fingerprints.
 */
...
struct metric_tanimoto_gt {
  ...
  static_assert(std::is_floating_point<result_t>::value,
                "Tanimoto distance will be a fraction");
...
#pragma GCC ivdep
    ...
    for (std::size_t i = 0; i != words; ++i)
      and_count += std::bitset<bits_per_word_k>(a[i] & b[i]).count(),
          any_count += std::bitset<bits_per_word_k>(a[i]).count() +
                       std::bitset<bits_per_word_k>(b[i]).count();
...
#pragma GCC ivdep
    ...
    result_t denom = std::sqrt((dim * a_sq_sum - a_sum * a_sum) *
...
/**
 *  @brief  Measures Jensen-Shannon Divergence between two probability
 * distributions.
 */
template <typename scalar_at = float, typename result_at = float>
struct metric_divergence_gt {
  ...
  inline result_t operator()(scalar_t const *p, scalar_t const *q,
                             std::size_t dim) const noexcept {
...
struct cos_i8_t {
  ...
  inline result_t operator()(i8_t const *a, i8_t const *b,
                             std::size_t dim) const noexcept {
...
struct metric_haversine_gt {
  using scalar_t = scalar_at;
  using result_t = result_at;
  static_assert(!std::is_integral<scalar_t>::value,
                "Latitude and longitude must be floating-node");

  inline result_t operator()(scalar_t const *a, scalar_t const *b,
                             std::size_t = 2) const noexcept {
    ...
    result_t x = square(std::sin(lat_delta)) + //
                 std::cos(converted_lat_a) * std::cos(converted_lat_b) *
                     square(std::sin(lon_delta));
 ...
 *          Can be just two array pointers, precompiled for a specific array
 * length, or include one or two array sizes as 64-bit unsigned integers.
 */
...
/**
 *  @brief  Type-punned metric class, which unlike STL's `std::function` avoids
 * any memory allocations. It also provides additional APIs to check, if SIMD
 * hardware-acceleration is available. Wraps the `simsimd_metric_punned_t`.
 */
...
public:
   ...
   *
   *  ! This is the only relevant function in the object. Everything else is
   * just dynamic dispatch logic.
   */
  inline result_t operator()(byte_t const *a, byte_t const *b) const noexcept {
    return raw_ptr_(reinterpret_cast<punned_arg_t>(a),
                    reinterpret_cast<punned_arg_t>(b), raw_arg3_, raw_arg4_);
  }
  ...
  inline std::size_t bytes_per_vector() const noexcept {
    return divide_round_up<CHAR_BIT>(dimensions_ *
                                     bits_per_scalar(scalar_kind_));
  }
...
private:
  ...
  void configure_with_auto_vectorized() noexcept {
    ...
    case metric_kind_t::l2sq_k: {
      ...
      case scalar_kind_t::f16_k:
        raw_ptr_ =
            (punned_ptr_t)&equidimensional_<metric_l2sq_gt<f16_t, f32_t>>;
        break;
      case scalar_kind_t::i8_k:
    ...
    case metric_kind_t::pearson_k: {
      ...
      case scalar_kind_t::i8_k:
        raw_ptr_ =
            (punned_ptr_t)&equidimensional_<metric_pearson_gt<i8_t, f32_t>>;
        break;
      case scalar_kind_t::f16_k:
        raw_ptr_ =
            (punned_ptr_t)&equidimensional_<metric_pearson_gt<f16_t, f32_t>>;
        break;
      case scalar_kind_t::f32_k:
    ...
    case metric_kind_t::haversine_k: {
      ...
      case scalar_kind_t::f16_k:
        raw_ptr_ =
            (punned_ptr_t)&equidimensional_<metric_haversine_gt<f16_t, f32_t>>;
        break;
      case scalar_kind_t::f32_k:
    ...
    case metric_kind_t::divergence_k: {
      ...
      case scalar_kind_t::f16_k:
        raw_ptr_ =
            (punned_ptr_t)&equidimensional_<metric_divergence_gt<f16_t, f32_t>>;
        break;
      case scalar_kind_t::f32_k:
...
/**
 *  @brief  View over a potentially-strided memory buffer, containing a
 * row-major matrix.
 */
...
public:
  ...
  vectors_view_gt(scalar_t const *begin, std::size_t dimensions,
                  std::size_t count = 1) noexcept
      : vectors_view_gt(begin, dimensions, count,
                        dimensions * sizeof(scalar_at)) {}

  vectors_view_gt(scalar_t const *begin, std::size_t dimensions,
                  std::size_t count, std::size_t stride_bytes) noexcept
      : begin_(begin), dimensions_(dimensions), count_(count),
  ...
  scalar_t const *at(std::size_t i) const noexcept {
    return reinterpret_cast<scalar_t const *>(
        reinterpret_cast<byte_t const *>(begin_) + i * stride_bytes_);
  }
};
...
public:
  ...
  exact_search_results_t operator()(                                          //
    ...
    return operator()( //
        metric,        //
        reinterpret_cast<byte_t const *>(dataset.data()), dataset.size(),
        dataset.stride(), //
        reinterpret_cast<byte_t const *>(queries.data()), queries.size(),
        queries.stride(), //
        wanted, executor, progress);
  }

  template <typename executor_at = dummy_executor_t,
            typename progress_at = dummy_progress_t>
  exact_search_results_t operator()( //
      byte_t const *dataset_data, std::size_t dataset_count,
      std::size_t dataset_stride, //
      byte_t const *queries_data, std::size_t queries_count,
      std::size_t queries_stride, //
      std::size_t wanted, metric_punned_t const &metric,
      executor_at &&executor = executor_at{},
      progress_at &&progress = progress_at{}) {

    // Allocate temporary memory to store the distance matrix
    // Previous version didn't need temporary memory, but the performance was
    // much lower. In the new design we keep two buffers - original and
    // transposed, as in-place transpositions of non-rectangular matrixes is
    // expensive.
    ...
    exact_offset_and_distance_t *keys_and_distances_per_dataset =
        keys_and_distances.data();
    ...
    executor.dynamic(dataset_count, [&](std::size_t thread_idx,
                                        std::size_t dataset_idx) {
      ...
      for (std::size_t query_idx = 0; query_idx != queries_count; ++query_idx) {
        ...
        keys_and_distances_per_dataset[task_idx].offset =
            static_cast<u32_t>(dataset_idx);
        keys_and_distances_per_dataset[task_idx].distance =
            static_cast<f32_t>(distance);
      }

      // It's more efficient in this case to report progress from a single
      // thread
    ...
    // §2. Transpose in a single thread to avoid contention writing into the
    // same memory buffers
    for (std::size_t query_idx = 0; query_idx != queries_count; ++query_idx) {
      for (std::size_t dataset_idx = 0; dataset_idx != dataset_count;
           ++dataset_idx) {
        std::size_t from_idx = queries_count * dataset_idx + query_idx;
        std::size_t to_idx = dataset_count * query_idx + dataset_idx;
        keys_and_distances_per_query[to_idx] =
            keys_and_distances_per_dataset[from_idx];
      }
    }
    ...
    executor.fixed(queries_count, [&](std::size_t, std::size_t query_idx) {
      ...
      if (wanted > 1) {
        ...
        auto min_it =
            std::min_element(start, start + dataset_count, &smaller_distance);
        if (min_it != start)
          std::swap(*min_it, *start);
      }
    });

    // At the end report the latest numbers, because the reporter thread may be
    // finished earlier
 ...
 *  @section Layout
 *
 *  For every slot we store 2 extra bits for 3 possible states: empty,
 * populated, or deleted. With linear probing the hashes at the end of the
 * populated region will spill into its first half.
 */
...
private:
  ...
  slot_ref_t slot_ref(char *data, std::size_t slot_index) const noexcept {
    ...
    auto slot_pointer = bucket_pointer + sizeof(bucket_header_t) +
                        sizeof(element_t) * in_bucket_index;
    return {
...
public:
  ...
  bool try_reserve(std::size_t capacity) noexcept {
    ...
    new_slots =
        new_buckets * slots_per_bucket(); // This must be a power of two!
  ...
  public:
    ...
    equal_iterator_gt(std::size_t index, flat_hash_multi_set_gt *parent,
                      query_at const &query, equals_t const &equals)
    ...
    equal_iterator_gt &operator++() {
      do {
        index_ = (index_ + 1) & (parent_->capacity_slots_ - 1);
               ...
               (parent_->slot_ref(index_).header.populated &
                parent_->slot_ref(index_).mask));
      return *this;
    }
  ...
  /**
   *  @brief  Returns an iterator range of all elements matching the given
   * query.
   *
  ...
  equal_range(query_at const &query) const noexcept {
    ...
    auto end =
        equal_iterator_gt<query_at>(capacity_slots_, this_ptr, query, equals);
    if (!capacity_slots_)
      return {end, end};
    ...
    // Start from the first matching element and find the end of the populated
    // range
    ...
    return {
        equal_iterator_gt<query_at>(first_equal_index, this_ptr, query, equals),
        equal_iterator_gt<query_at>(first_empty_index, this_ptr, query,
                                    equals)};
  }
  ...
  bool pop_first(similar_at &&query, element_t &popped_value) noexcept {
    ...
    std::size_t slot_index =
        hash_value &
        (capacity_slots_ - 1); // Assuming capacity_slots_ is a power of 2
  ...
  template <typename func_at> void for_each(func_at &&func) const {
    for (std::size_t bucket_index = 0; bucket_index < buckets_;
         ++bucket_index) {
      ...
      for (std::size_t in_bucket_index = 0;
           in_bucket_index < slots_per_bucket(); ++in_bucket_index) {
        ...
        if ((populated & ~deleted) & mask) {
          ...
          element_t const &element =
              *reinterpret_cast<element_t const *>(slot_pointer);
          func(element);
        }
      }
    }
