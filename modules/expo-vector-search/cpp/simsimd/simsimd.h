 ...
 *  References:
 *  x86 intrinsics:
 * https://www.intel.com/content/www/us/en/docs/intrinsics-guide Arm intrinsics:
 * https://developer.arm.com/architectures/instruction-sets/intrinsics Detecting
 * target CPU features at compile time:
 * https://stackoverflow.com/a/28939692/2766161
 *
 *  @section    Choosing x86 Target Generations
 *
 *  It's important to provide fine-grained controls over AVX512 families, as
 ...
 *         F, CD, VL, DQ, BW, VNNI, VPOPCNTDQ, IFMA, VBMI, VAES, GFNI, VBMI2,
 * BITALG, VPCLMULQDQ
 *  - AMD Zen4 (Genoa):
 *         F, CD, VL, DQ, BW, VNNI, VPOPCNTDQ, IFMA, VBMI, VAES, GFNI, VBMI2,
 * BITALG, VPCLMULQDQ, BF16 > In other words, it extends Sunny Cove with BF16
 * support
 *  - Intel Golden Cove (Sapphire Rapids): extends Zen4 and Sunny Cove with FP16
 * support
 *  - AMD Zen5 (Turin): makes VP2INTERSECT cool again
 *
 *  Intel Palm Cove was an irrelevant intermediate release extending Skylake
 * with IFMA and VBMI. Intel Willow Cove was an irrelevant intermediate release
 * extending Sunny Cove with VP2INTERSECT, which are not supported by other CPUs
 * to date and are only available in Tiger Lake laptops. Intel Cooper Lake was
 * the only intermediary platform, that supported BF16, but not FP16. It's
 * mostly used in 4-socket and 8-socket high-memory configurations.
 *
 ...
 *
 *  Beyond those, we support AVX2 for old Haswell generation CPUs, and AVX2+VNNI
 * for modern Sierra generation.
 *
 *  To list all available macros for x86, take a recent compiler, like GCC 12
 * and run: gcc-12 -march=sapphirerapids -dM -E - < /dev/null | egrep "SSE|AVX"
 * | sort On Arm machines you may want to check for other flags: gcc-12
 * -march=native -dM -E - < /dev/null | egrep "NEON|SVE|FP16|FMA" | sort
 *
 *  @section    Choosing Arm Target Generations
 *
 *  Arm CPUs share design IP, but are produced by different vendors, potentially
 * making the platform even more fragmented than x86. There are 2 important
 * families of SIMD extensions - NEON and SVE.
 *
 *  - Armv8-A: +fp, +simd
 ...
 *  A 512-bit SVE variant has already been implemented on the Fugaku
 * supercomputer. A more flexible version, 2x256 SVE, was implemented by the AWS
 * Graviton3 ARM processor. Here are the most important recent families of CPU
 ...
 *  - Neoverse V1: armv8.4-a, extended with Armv8.6 bfloat/int8 "matmul"
 * instructions. Used in AWS @b Graviton3, which also enables `sve`, `svebf16`,
 * and `svei8mm`. https://developer.arm.com/Processors/Neoverse%20V1
 ...
 *
 *  On the consumer side, Apple is the biggest player with mobile @b A chips and
 * desktop @b M chips. The M1 implements Armv8.5-A, both M2 and M3 implement
 * Armv8.6-A, and M4 is expected to have Armv9.1-A.
 */
...
/**
 *  @brief  Removes compile-time dispatching, and replaces it with runtime
 * dispatching. So the `simsimd_dot_f32` function will invoke the most advanced
 * backend supported by the CPU, that runs the program, rather than the most
 * advanced backend supported by the CPU used to compile the library or the
...
// On Apple Silicon, `mrs` is not allowed in user-space, so we need to use the
// `sysctl` API.
...
// POSIX extensions provide `sigaction`, `sigjmp_buf`, and `sigsetjmp` for safe
// signal handling. These are needed on Linux ARM for safely testing `mrs`
...
typedef enum {
  ...
  simsimd_metric_spdot_weights_k =
  ...
  simsimd_metric_kullback_leibler_k =
      'k', ///< Kullback-Leibler divergence alias
...
typedef enum {
  ...
  simsimd_cap_any_k =
      0x7FFFFFFF, ///< Mask representing any capability with `INT_MAX`

  simsimd_cap_haswell_k =
      1 << 10, ///< x86 AVX2 capability with FMA and F16C extensions
  simsimd_cap_skylake_k = 1 << 11, ///< x86 AVX512 baseline capability
  simsimd_cap_ice_k =
      1 << 12, ///< x86 AVX512 capability with advanced integer algos
  simsimd_cap_genoa_k = 1 << 13, ///< x86 AVX512 capability with `bf16` support
  simsimd_cap_sapphire_k = 1
                           << 14, ///< x86 AVX512 capability with `f16` support
  simsimd_cap_turin_k =
      1 << 15, ///< x86 AVX512 capability with conflict detection
  simsimd_cap_sierra_k =
      1 << 16, ///< x86 AVX2+VNNI capability with `i8` dot-products
 ...
 *
 *  Includes complex type descriptors which in C code would use the real
 * counterparts, but the independent flags contain metadata to be passed between
 * programming language interfaces.
 */
typedef enum {
  simsimd_datatype_unknown_k = 0, ///< Unknown data type
  simsimd_datatype_b8_k = 1 << 1, ///< Single-bit values packed into 8-bit words
  simsimd_datatype_b1x8_k =
      simsimd_datatype_b8_k, ///< Single-bit values packed into 8-bit words
  simsimd_datatype_i4x2_k =
      1 << 19, ///< 4-bit signed integers packed into 8-bit words
  ...
  simsimd_datatype_f64c_k = 1
                            << 20, ///< Complex double precision floating point
  simsimd_datatype_f32c_k = 1
                            << 21, ///< Complex single precision floating point
...
/**
 *  @brief  Type-punned function pointer for dense vector representations and
 ...
 *                  When dealing with sub-byte types, the number of scalar words
 * is the number of bytes. When dealing with complex types, the number of scalar
 * words is the sum of real and imaginary parts.
 *  @param[out] d   Output value as a double-precision float.
 *                  In complex dot-products @b two scalars are exported for the
 * real and imaginary parts.
 */
typedef void (*simsimd_metric_dense_punned_t)(void const *a, void const *b,
                                              simsimd_size_t n,
                                              simsimd_distance_t *d);

/**
 *  @brief  Type-punned function pointer for sparse vector representations and
 * similarity measures.
 *
 *  @param[in] a          Pointer to the first data array, generally a sorted
 * array of integers.
 *  @param[in] b          Pointer to the second data array, generally a sorted
 * array of integers.
 ...
 *  @param[out] d         Output value as a double-precision float, generally
 * without decimals.
 */
...
/**
 *  @brief  Type-punned function pointer for curved vector spaces and similarity
 * measures.
 *
 *  @param[in] a    Pointer to the first data array.
 *  @param[in] b    Pointer to the second data array.
 *  @param[in] c    Pointer to the metric tensor array or some covariance
 * matrix.
 *  @param[in] n    Number of scalar words in the input arrays.
 *  @param[out] d   Output value as a double-precision float.
 */
...
/**
 *  @brief  Type-punned function pointer for FMA operations on dense vector
 * representations. Implements the `y = alpha * a * b + beta * c` operation.
 *
 *  @param[in] a        Pointer to the first data array.
 *  @param[in] b        Pointer to the second data array.
...
/**
 *  @brief  Type-punned function pointer for Weighted Sum operations on dense
 * vector representations. Implements the `y = alpha * a + beta * b` operation.
 *
 *  @param[in] a        Pointer to the first data array.
 *  @param[in] b        Pointer to the second data array.
 *  @param[in] n        Number of scalar words in the input arrays.
 ...
 * `simsimd_metric_curved_punned_t`, `simsimd_kernel_fma_punned_t`, or
 * `simsimd_kernel_wsum_punned_t`.
 */
 ...
 *  @note   This should be called on each thread before any SIMD operations to
...
/**
 *  @brief  Function to determine the SIMD capabilities of the current 64-bit
 * x86 machine at @b runtime.
 *  @return A bitmask of the SIMD capabilities represented as a
 * `simsimd_capability_t` enum value.
 */
SIMSIMD_PUBLIC simsimd_capability_t _simsimd_capabilities_x86(void) {
  ...
  __asm__ __volatile__( //
      "cpuid"
      : "=a"(info1.named.eax), "=b"(info1.named.ebx), "=c"(info1.named.ecx),
        "=d"(info1.named.edx)
      : "a"(1), "c"(0));
  __asm__ __volatile__( //
      "cpuid"
      : "=a"(info7.named.eax), "=b"(info7.named.ebx), "=c"(info7.named.ecx),
        "=d"(info7.named.edx)
      : "a"(7), "c"(0));
  __asm__ __volatile__( //
      "cpuid"
      : "=a"(info7sub1.named.eax), "=b"(info7sub1.named.ebx),
        "=c"(info7sub1.named.ecx), "=d"(info7sub1.named.edx)
      : "a"(7), "c"(1));
#endif
  ...
  // We don't want to accidentally enable AVX512VP2INTERSECT on Intel Tiger Lake
  // CPUs
...
/*  Compiling the next section one may get: selected processor does not support
 * system register name 'id_aa64zfr0_el1'. Suppressing assembler errors is very
 * complicated, so when dealing with older ARM CPUs it's simpler to compile this
 *  function targeting newer ones.
 */
#pragma GCC push_options
#pragma GCC target("arch=armv8.5-a+sve")
#pragma clang attribute push(__attribute__((target("arch=armv8.5-a+sve"))),    \
                             apply_to = function)
 ...
 *  @note   This should be called on each thread before any SIMD operations to
 * avoid performance penalties.
 *  @note   On Apple Silicon, `mrs` is not allowed in user-space, so we need to
 * use the `sysctl` API.
 *  @return 1 if the operation was successful, 0 otherwise.
 */
SIMSIMD_PUBLIC int _simsimd_flush_denormals_arm(void) {
  ...
  fpcr |=
      (1
       << 25); // bit 25 = DN (Force Default NaN instead of preserving payload)
...
SIMSIMD_PUBLIC simsimd_capability_t _simsimd_capabilities_arm(void) {
  ...
  // On Apple Silicon, `mrs` is not allowed in user-space, so we need to use the
  // `sysctl` API.
  unsigned supports_neon = 0, supports_fp16 = 0, supports_bf16 = 0,
           supports_i8mm = 0, supports_dotprod = 0;
  size_t size = sizeof(supports_neon);
  if (sysctlbyname("hw.optional.neon", &supports_neon, &size, NULL, 0) != 0)
    supports_neon = 0;
  if (sysctlbyname("hw.optional.arm.FEAT_FP16", &supports_fp16, &size, NULL,
                   0) != 0)
    supports_fp16 = 0;
  if (sysctlbyname("hw.optional.arm.FEAT_BF16", &supports_bf16, &size, NULL,
                   0) != 0)
    supports_bf16 = 0;
  if (sysctlbyname("hw.optional.arm.FEAT_I8MM", &supports_i8mm, &size, NULL,
                   0) != 0)
    supports_i8mm = 0;
  if (sysctlbyname("hw.optional.arm.FEAT_DotProd", &supports_dotprod, &size,
                   NULL, 0) != 0)
    supports_dotprod = 0;
  ...
  return (simsimd_capability_t)(                                     //
      ...
      (simsimd_cap_neon_i8_k *
       (supports_neon && (supports_i8mm || supports_dotprod))) | //
      (simsimd_cap_serial_k));

#elif defined(_SIMSIMD_DEFINED_LINUX)

  // Depending on the environment, reading system registers may cause SIGILL.
  // One option to avoid the crash is to use `getauxval(AT_HWCAP)` and
  // `getauxval(AT_HWCAP2)`, Linux APIs, but those aren't as informative as
  // reading the registers directly. So before reading the ID registers, we set
  // up a signal handler to catch SIGILL and probe one of the registers,
  // reverting back to the old signal handler afterwards.
  //
  // This issue was originally observed in:
  // https://github.com/ashvardanian/SimSIMD/issues/279
#if _SIMSIMD_HAS_POSIX_EXTENSIONS
  ...
  // Early exit if `mrs` doesn't work - return conservative NEON-only
  // capabilities
  if (!mrs_works)
  ...
  unsigned long id_aa64isar0_el1 = 0, id_aa64isar1_el1 = 0, id_aa64pfr0_el1 = 0,
                id_aa64zfr0_el1 = 0;
  ...
  unsigned supports_integer_dot_products =
      ((id_aa64isar0_el1 >> 44) & 0xF) >= 1;
  ...
  // AdvSIMD, bits [23:20] of ID_AA64PFR0_EL1 can be used to check for `fp16`
  // support
  ...
  // That's a really weird way to encode lack of NEON support, but it's
  // important to check in case we are running on R-profile CPUs.
  ...
  return (simsimd_capability_t)(                                     //
      ...
      (simsimd_cap_neon_bf16_k * (supports_neon && supports_bf16)) | //
      (simsimd_cap_neon_i8_k *
       (supports_neon && supports_i8mm && supports_integer_dot_products)) | //
  ...
  // On Windows ARM, use the `IsProcessorFeaturePresent` API for capability
  // detection.
  ...
  // Conservative fallback for unknown platforms: NEON is mandatory in ARMv8-A
  // (ARM64)
  return (simsimd_capability_t)(simsimd_cap_neon_k | simsimd_cap_serial_k);
...
/**
 *  @brief  Function to flush @b denormalized numbers to zero to avoid
 * performance penalties.
 *  @return 1 if the operation was successful, 0 otherwise.
 *
 *  When facing denormalized values Fused-Multiply-Add (FMA) operations can be
 * up to 30x slower, as measured on Intel Sapphire Rapids:
...
SIMSIMD_INTERNAL void
_simsimd_find_kernel_punned_f64(simsimd_capability_t v, simsimd_metric_kind_t k,
                                simsimd_kernel_punned_t *m,
                                simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
#if SIMSIMD_TARGET_SVE
  if (v & simsimd_cap_sve_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f64_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f64_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f64_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f64_sve, *c = simsimd_cap_sve_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_k)
    switch (k) {
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f64_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f64_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f64_neon, *c = simsimd_cap_neon_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_skylake_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f64_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f64_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f64_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f64_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f64_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_wsum_k:
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f64_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f64_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f64_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f64_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f64_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f64_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f64_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void
_simsimd_find_kernel_punned_f32(simsimd_capability_t v, simsimd_metric_kind_t k,
                                simsimd_kernel_punned_t *m,
                                simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
#if SIMSIMD_TARGET_SVE
  if (v & simsimd_cap_sve_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f32_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f32_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f32_sve, *c = simsimd_cap_sve_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f32_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f32_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f32_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_f32_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_f32_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f32_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f32_neon, *c = simsimd_cap_neon_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_skylake_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f32_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_wsum_k:
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f32_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f32_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f32_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f32_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f32_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f32_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f32_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void
_simsimd_find_kernel_punned_f16(simsimd_capability_t v, simsimd_metric_kind_t k,
                                simsimd_kernel_punned_t *m,
                                simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
  ...
  if (v & simsimd_cap_sve_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16_sve, *c = simsimd_cap_sve_f16_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f16_sve, *c = simsimd_cap_sve_f16_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f16_sve, *c = simsimd_cap_sve_f16_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f16_sve, *c = simsimd_cap_sve_f16_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_f16_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f16_neon, *c = simsimd_cap_neon_f16_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_sapphire_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_bilinear_k:
    ...
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f16_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f16_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_f16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_f16_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_bf16(
    simsimd_capability_t v, simsimd_metric_kind_t k, simsimd_kernel_punned_t *m,
    simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
  ...
  if (v & simsimd_cap_sve_bf16_k)
    switch (k) {
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_bf16_sve, *c = simsimd_cap_sve_bf16_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_bf16_sve, *c = simsimd_cap_sve_bf16_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_bf16_sve, *c = simsimd_cap_sve_bf16_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_bf16_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_bf16_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_bf16_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_bf16_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_bf16_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_bf16_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_bf16_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_genoa_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_bf16_genoa, *c = simsimd_cap_genoa_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_bf16_genoa, *c = simsimd_cap_genoa_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_bf16_genoa, *c = simsimd_cap_genoa_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_bf16_genoa, *c = simsimd_cap_genoa_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_bf16_genoa, *c = simsimd_cap_genoa_k;
      return;
    case simsimd_metric_mahalanobis_k:
  ...
  if (v & simsimd_cap_skylake_k)
    switch (k) {
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_bf16_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_wsum_k:
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_bf16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_bf16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_bf16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_bf16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_bilinear_k:
    ...
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_bf16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_bf16_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_bf16_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_js_k:
      *m = (m_t)&simsimd_js_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_kl_k:
      *m = (m_t)&simsimd_kl_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_mahalanobis_k:
      *m = (m_t)&simsimd_mahalanobis_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_bf16_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_i8(simsimd_capability_t v,
                                                     simsimd_metric_kind_t k,
                                                     simsimd_kernel_punned_t *m,
                                                     simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
  ...
  if (v & simsimd_cap_neon_i8_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_i8_neon, *c = simsimd_cap_neon_i8_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_i8_neon, *c = simsimd_cap_neon_i8_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_i8_neon, *c = simsimd_cap_neon_i8_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_i8_neon, *c = simsimd_cap_neon_i8_k;
      return;
    default:
      break;
    }
#endif
#if SIMSIMD_TARGET_NEON_F16 //! Scaling of 8-bit integers is performed using
                            //! 16-bit floats.
  if (v & simsimd_cap_neon_f16_k)
    switch (k) {
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_i8_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_i8_neon, *c = simsimd_cap_neon_f16_k;
      return;
    default:
      break;
    }
#endif
#if SIMSIMD_TARGET_SAPPHIRE //! Scaling of 8-bit integers is performed using
                            //! 16-bit floats.
  if (v & simsimd_cap_sapphire_k)
    switch (k) {
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_i8_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_i8_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_ice_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_i8_ice, *c = simsimd_cap_ice_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_i8_ice, *c = simsimd_cap_ice_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_i8_ice, *c = simsimd_cap_ice_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_i8_ice, *c = simsimd_cap_ice_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_i8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_i8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_i8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_i8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_i8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_i8_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_i8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_i8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_i8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_i8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_i8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_i8_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
}
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_u8(simsimd_capability_t v,
                                                     simsimd_metric_kind_t k,
                                                     simsimd_kernel_punned_t *m,
                                                     simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
  ...
  if (v & simsimd_cap_neon_i8_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_u8_neon, *c = simsimd_cap_neon_i8_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_u8_neon, *c = simsimd_cap_neon_i8_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_u8_neon, *c = simsimd_cap_neon_i8_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_u8_neon, *c = simsimd_cap_neon_i8_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_f16_k)
    switch (k) {
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_u8_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_u8_neon, *c = simsimd_cap_neon_f16_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_sapphire_k)
    switch (k) {
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_u8_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_u8_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_ice_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_u8_ice, *c = simsimd_cap_ice_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_u8_ice, *c = simsimd_cap_ice_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_u8_ice, *c = simsimd_cap_ice_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_u8_ice, *c = simsimd_cap_ice_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_u8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_u8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_u8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_u8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_u8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_u8_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_u8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_cos_k:
      *m = (m_t)&simsimd_cos_u8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2sq_k:
      *m = (m_t)&simsimd_l2sq_u8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_l2_k:
      *m = (m_t)&simsimd_l2_u8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_fma_k:
      *m = (m_t)&simsimd_fma_u8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_wsum_k:
      *m = (m_t)&simsimd_wsum_u8_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_b8(simsimd_capability_t v,
                                                     simsimd_metric_kind_t k,
                                                     simsimd_kernel_punned_t *m,
                                                     simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
  ...
  if (v & simsimd_cap_sve_k)
    switch (k) {
    case simsimd_metric_hamming_k:
      *m = (m_t)&simsimd_hamming_b8_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_jaccard_k:
      *m = (m_t)&simsimd_jaccard_b8_sve, *c = simsimd_cap_sve_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_k)
    switch (k) {
    case simsimd_metric_hamming_k:
      *m = (m_t)&simsimd_hamming_b8_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_jaccard_k:
      *m = (m_t)&simsimd_jaccard_b8_neon, *c = simsimd_cap_neon_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_ice_k)
    switch (k) {
    case simsimd_metric_hamming_k:
      *m = (m_t)&simsimd_hamming_b8_ice, *c = simsimd_cap_ice_k;
      return;
    case simsimd_metric_jaccard_k:
      *m = (m_t)&simsimd_jaccard_b8_ice, *c = simsimd_cap_ice_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_hamming_k:
      *m = (m_t)&simsimd_hamming_b8_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_jaccard_k:
      *m = (m_t)&simsimd_jaccard_b8_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_hamming_k:
      *m = (m_t)&simsimd_hamming_b8_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_jaccard_k:
      *m = (m_t)&simsimd_jaccard_b8_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_f64c(
    simsimd_capability_t v, simsimd_metric_kind_t k, simsimd_kernel_punned_t *m,
    simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
#if SIMSIMD_TARGET_SVE
  if (v & simsimd_cap_sve_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f64c_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f64c_sve, *c = simsimd_cap_sve_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_skylake_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f64c_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f64c_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f64c_skylake, *c = simsimd_cap_skylake_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f64c_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f64c_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f64c_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_f32c(
    simsimd_capability_t v, simsimd_metric_kind_t k, simsimd_kernel_punned_t *m,
    simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
#if SIMSIMD_TARGET_SVE
  if (v & simsimd_cap_sve_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32c_sve, *c = simsimd_cap_sve_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f32c_sve, *c = simsimd_cap_sve_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32c_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f32c_neon, *c = simsimd_cap_neon_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f32c_neon, *c = simsimd_cap_neon_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_skylake_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32c_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f32c_skylake, *c = simsimd_cap_skylake_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f32c_skylake, *c = simsimd_cap_skylake_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32c_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f32c_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f32c_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f32c_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f32c_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_f16c(
    simsimd_capability_t v, simsimd_metric_kind_t k, simsimd_kernel_punned_t *m,
    simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
#if SIMSIMD_TARGET_SVE_F16
  if (v & simsimd_cap_sve_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16c_sve, *c = simsimd_cap_sve_f16_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f16c_sve, *c = simsimd_cap_sve_f16_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_f16_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16c_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f16c_neon, *c = simsimd_cap_neon_f16_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f16c_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_sapphire_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16c_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f16c_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f16c_sapphire, *c = simsimd_cap_sapphire_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_haswell_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16c_haswell, *c = simsimd_cap_haswell_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f16c_haswell, *c = simsimd_cap_haswell_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_f16c_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_f16c_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_f16c_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_bf16c(
    simsimd_capability_t v, simsimd_metric_kind_t k, simsimd_kernel_punned_t *m,
    simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
#if SIMSIMD_TARGET_NEON_BF16
  if (v & simsimd_cap_neon_bf16_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_bf16c_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_bf16c_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_bf16c_neon, *c = simsimd_cap_neon_bf16_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_genoa_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_bf16c_genoa, *c = simsimd_cap_genoa_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_bf16c_genoa, *c = simsimd_cap_genoa_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_bf16c_genoa, *c = simsimd_cap_genoa_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_dot_k:
      *m = (m_t)&simsimd_dot_bf16c_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_vdot_k:
      *m = (m_t)&simsimd_vdot_bf16c_serial, *c = simsimd_cap_serial_k;
      return;
    case simsimd_metric_bilinear_k:
      *m = (m_t)&simsimd_bilinear_bf16c_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void
_simsimd_find_kernel_punned_u16(simsimd_capability_t v, simsimd_metric_kind_t k,
                                simsimd_kernel_punned_t *m,
                                simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
  ...
  if (v & simsimd_cap_sve2_k)
    ...
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u16_sve2, *c = simsimd_cap_sve2_k;
      return;
  ...
  if (v & simsimd_cap_neon_k)
    switch (k) {
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u16_neon, *c = simsimd_cap_neon_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_turin_k)
    switch (k) {
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u16_turin, *c = simsimd_cap_turin_k;
      return;
    case simsimd_metric_spdot_counts_k:
  ...
  if (v & simsimd_cap_ice_k)
    switch (k) {
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u16_ice, *c = simsimd_cap_skylake_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u16_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
SIMSIMD_INTERNAL void
_simsimd_find_kernel_punned_u32(simsimd_capability_t v, simsimd_metric_kind_t k,
                                simsimd_kernel_punned_t *m,
                                simsimd_capability_t *c) {
  typedef simsimd_kernel_punned_t m_t;
#if SIMSIMD_TARGET_SVE2
  if (v & simsimd_cap_sve2_k)
    switch (k) {
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u32_sve2, *c = simsimd_cap_sve2_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_neon_k)
    switch (k) {
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u32_neon, *c = simsimd_cap_neon_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_ice_k)
    switch (k) {
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u32_ice, *c = simsimd_cap_skylake_k;
      return;
    default:
      break;
    }
  ...
  if (v & simsimd_cap_serial_k)
    switch (k) {
    case simsimd_metric_intersect_k:
      *m = (m_t)&simsimd_intersect_u32_serial, *c = simsimd_cap_serial_k;
      return;
    default:
      break;
    }
...
/**
 *  @brief  Determines the best suited metric implementation based on the given
 * datatype, supported and allowed by hardware capabilities.
 *
 ...
 *  @param capability_output Output variable for the utilized hardware
 * capabilities.
 */
SIMSIMD_INTERNAL void _simsimd_find_kernel_punned_implementation( //
  ...
  case simsimd_datatype_f64_k:
    _simsimd_find_kernel_punned_f64(viable, kind, m, c);
    return;
  case simsimd_datatype_f32_k:
    _simsimd_find_kernel_punned_f32(viable, kind, m, c);
    return;
  case simsimd_datatype_f16_k:
    _simsimd_find_kernel_punned_f16(viable, kind, m, c);
    return;
  case simsimd_datatype_bf16_k:
    _simsimd_find_kernel_punned_bf16(viable, kind, m, c);
    return;
  case simsimd_datatype_i8_k:
    _simsimd_find_kernel_punned_i8(viable, kind, m, c);
    return;
  case simsimd_datatype_u8_k:
    _simsimd_find_kernel_punned_u8(viable, kind, m, c);
    return;
  case simsimd_datatype_b8_k:
    _simsimd_find_kernel_punned_b8(viable, kind, m, c);
    return;
  case simsimd_datatype_f32c_k:
    _simsimd_find_kernel_punned_f32c(viable, kind, m, c);
    return;
  case simsimd_datatype_f64c_k:
    _simsimd_find_kernel_punned_f64c(viable, kind, m, c);
    return;
  case simsimd_datatype_f16c_k:
    _simsimd_find_kernel_punned_f16c(viable, kind, m, c);
    return;
  case simsimd_datatype_bf16c_k:
    _simsimd_find_kernel_punned_bf16c(viable, kind, m, c);
    return;
  case simsimd_datatype_u16_k:
    _simsimd_find_kernel_punned_u16(viable, kind, m, c);
    return;
  case simsimd_datatype_u32_k:
    _simsimd_find_kernel_punned_u32(viable, kind, m, c);
    return;
  ...
  case simsimd_datatype_u64_k:
    break;
  case simsimd_datatype_unknown_k:
    break;
  default:
    break;
  }
...
/**
 *  @brief  Selects the most suitable metric implementation based on the given
 * metric kind, datatype, and allowed capabilities. @b Don't call too often and
 * prefer caching the `simsimd_capabilities()`.
 *
 *  @param kind The kind of metric to be evaluated.
 *  @param datatype The data type for which the metric needs to be evaluated.
 *  @param allowed The hardware capabilities allowed for use.
 ...
 *  - Check if the CPU supports AVX2 and F16C extensions on Haswell x86 CPUs and
 * newer
 *  - Check if the CPU supports AVX512F and AVX512BW extensions on Skylake x86
 * CPUs and newer
 *  - Check if the CPU supports AVX512VNNI, AVX512IFMA, AVX512BITALG,
 * AVX512VBMI2, and AVX512VPOPCNTDQ extensions on Ice Lake x86 CPUs and newer
 *  - Check if the CPU supports AVX512BF16 extensions on Genoa x86 CPUs and
 * newer
 *  - Check if the CPU supports AVX512FP16 extensions on Sapphire Rapids x86
 * CPUs and newer
 *  - Check if the CPU supports AVX2VP2INTERSECT extensions on Turin x86 CPUs
 * and newer
 *
...
/*  Inner products
 *  - Dot product: the sum of the products of the corresponding elements of two
 * vectors.
 *  - Complex Dot product: dot product with a conjugate first argument.
 *  - Complex Conjugate Dot product: dot product with a conjugate first
 * argument.
 *
 ...
 *
 *  @note The dot product can be negative, to use as a distance, take `1 - a *
 * b`.
 ...
 */
SIMSIMD_DYNAMIC void simsimd_dot_i8(simsimd_i8_t const *a,
                                    simsimd_i8_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_u8(simsimd_u8_t const *a,
                                    simsimd_u8_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_f16(simsimd_f16_t const *a,
                                     simsimd_f16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_bf16(simsimd_bf16_t const *a,
                                      simsimd_bf16_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_f32(simsimd_f32_t const *a,
                                     simsimd_f32_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_f64(simsimd_f64_t const *a,
                                     simsimd_f64_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_f16c(simsimd_f16c_t const *a,
                                      simsimd_f16c_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_bf16c(simsimd_bf16c_t const *a,
                                       simsimd_bf16c_t const *b,
                                       simsimd_size_t n, simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_f32c(simsimd_f32c_t const *a,
                                      simsimd_f32c_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_dot_f64c(simsimd_f64c_t const *a,
                                      simsimd_f64c_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_vdot_f16c(simsimd_f16c_t const *a,
                                       simsimd_f16c_t const *b,
                                       simsimd_size_t n, simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_vdot_bf16c(simsimd_bf16c_t const *a,
                                        simsimd_bf16c_t const *b,
                                        simsimd_size_t n,
                                        simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_vdot_f32c(simsimd_f32c_t const *a,
                                       simsimd_f32c_t const *b,
                                       simsimd_size_t n, simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_vdot_f64c(simsimd_f64c_t const *a,
                                       simsimd_f64c_t const *b,
                                       simsimd_size_t n, simsimd_distance_t *d);
 ...
 *  @note The output distance value is zero if and only if the two vectors are
 * identical.
 *  @note Defined only for floating-point and integer data types.
 */
SIMSIMD_DYNAMIC void simsimd_cos_i8(simsimd_i8_t const *a,
                                    simsimd_i8_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_cos_u8(simsimd_u8_t const *a,
                                    simsimd_u8_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_cos_f16(simsimd_f16_t const *a,
                                     simsimd_f16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_cos_bf16(simsimd_bf16_t const *a,
                                      simsimd_bf16_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_cos_f32(simsimd_f32_t const *a,
                                     simsimd_f32_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_cos_f64(simsimd_f64_t const *a,
                                     simsimd_f64_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2sq_i8(simsimd_i8_t const *a,
                                     simsimd_i8_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2sq_u8(simsimd_u8_t const *a,
                                     simsimd_u8_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2sq_f16(simsimd_f16_t const *a,
                                      simsimd_f16_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2sq_bf16(simsimd_bf16_t const *a,
                                       simsimd_bf16_t const *b,
                                       simsimd_size_t n, simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2sq_f32(simsimd_f32_t const *a,
                                      simsimd_f32_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2sq_f64(simsimd_f64_t const *a,
                                      simsimd_f64_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2_i8(simsimd_i8_t const *a, simsimd_i8_t const *b,
                                   simsimd_size_t n, simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2_u8(simsimd_u8_t const *a, simsimd_u8_t const *b,
                                   simsimd_size_t n, simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2_f16(simsimd_f16_t const *a,
                                    simsimd_f16_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2_bf16(simsimd_bf16_t const *a,
                                     simsimd_bf16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2_f32(simsimd_f32_t const *a,
                                    simsimd_f32_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_l2_f64(simsimd_f64_t const *a,
                                    simsimd_f64_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);

/*  Binary distances
 *  - Hamming distance: the number of positions at which the corresponding bits
 * are different.
 *  - Jaccard distance: ratio of bit-level matching positions (intersection) to
 * the total number of positions (union).
 *
 ...
 */
SIMSIMD_DYNAMIC void simsimd_hamming_b8(simsimd_b8_t const *a,
                                        simsimd_b8_t const *b, simsimd_size_t n,
                                        simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_jaccard_b8(simsimd_b8_t const *a,
                                        simsimd_b8_t const *b, simsimd_size_t n,
                                        simsimd_distance_t *d);

/*  Probability distributions
 *  - Jensen-Shannon divergence: a measure of similarity between two probability
 * distributions.
 ...
 *  @note The output divergence value is zero if and only if the two
 * distributions are identical.
 *  @note Defined only for floating-point data types.
 */
SIMSIMD_DYNAMIC void simsimd_kl_f16(simsimd_f16_t const *a,
                                    simsimd_f16_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_kl_bf16(simsimd_bf16_t const *a,
                                     simsimd_bf16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_kl_f32(simsimd_f32_t const *a,
                                    simsimd_f32_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_kl_f64(simsimd_f64_t const *a,
                                    simsimd_f64_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_js_f16(simsimd_f16_t const *a,
                                    simsimd_f16_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_js_bf16(simsimd_bf16_t const *a,
                                     simsimd_bf16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_js_f32(simsimd_f32_t const *a,
                                    simsimd_f32_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
SIMSIMD_DYNAMIC void simsimd_js_f64(simsimd_f64_t const *a,
                                    simsimd_f64_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d);
 ...
 *  @note Defined only for floating-point and integer data types.
 */
SIMSIMD_PUBLIC void simsimd_dot_i8(simsimd_i8_t const *a, simsimd_i8_t const *b,
                                   simsimd_size_t n, simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_I8
...
SIMSIMD_PUBLIC void simsimd_dot_u8(simsimd_u8_t const *a, simsimd_u8_t const *b,
                                   simsimd_size_t n, simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_I8
...
SIMSIMD_PUBLIC void simsimd_dot_f16(simsimd_f16_t const *a,
                                    simsimd_f16_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE_F16
...
SIMSIMD_PUBLIC void simsimd_dot_bf16(simsimd_bf16_t const *a,
                                     simsimd_bf16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d) {
#if SIMSIMD_TARGET_GENOA
...
SIMSIMD_PUBLIC void simsimd_dot_f32(simsimd_f32_t const *a,
                                    simsimd_f32_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_dot_f64(simsimd_f64_t const *a,
                                    simsimd_f64_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_dot_f16c(simsimd_f16c_t const *a,
                                     simsimd_f16c_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE_F16
...
SIMSIMD_PUBLIC void simsimd_dot_bf16c(simsimd_bf16c_t const *a,
                                      simsimd_bf16c_t const *b,
                                      simsimd_size_t n, simsimd_distance_t *d) {
#if SIMSIMD_TARGET_GENOA
...
SIMSIMD_PUBLIC void simsimd_dot_f32c(simsimd_f32c_t const *a,
                                     simsimd_f32c_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_dot_f64c(simsimd_f64c_t const *a,
                                     simsimd_f64c_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_vdot_f16c(simsimd_f16c_t const *a,
                                      simsimd_f16c_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_vdot_bf16c(simsimd_bf16c_t const *a,
                                       simsimd_bf16c_t const *b,
                                       simsimd_size_t n,
                                       simsimd_distance_t *d) {
#if SIMSIMD_TARGET_GENOA
...
SIMSIMD_PUBLIC void simsimd_vdot_f32c(simsimd_f32c_t const *a,
                                      simsimd_f32c_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_vdot_f64c(simsimd_f64c_t const *a,
                                      simsimd_f64c_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
 ...
 *  @note Defined only for floating-point and integer data types.
 */
SIMSIMD_PUBLIC void simsimd_cos_i8(simsimd_i8_t const *a, simsimd_i8_t const *b,
                                   simsimd_size_t n, simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_I8
...
SIMSIMD_PUBLIC void simsimd_cos_u8(simsimd_u8_t const *a, simsimd_u8_t const *b,
                                   simsimd_size_t n, simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_I8
...
SIMSIMD_PUBLIC void simsimd_cos_f16(simsimd_f16_t const *a,
                                    simsimd_f16_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE_F16
...
SIMSIMD_PUBLIC void simsimd_cos_bf16(simsimd_bf16_t const *a,
                                     simsimd_bf16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d) {
#if SIMSIMD_TARGET_GENOA
...
SIMSIMD_PUBLIC void simsimd_cos_f32(simsimd_f32_t const *a,
                                    simsimd_f32_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_cos_f64(simsimd_f64_t const *a,
                                    simsimd_f64_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_l2sq_i8(simsimd_i8_t const *a,
                                    simsimd_i8_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_I8
...
SIMSIMD_PUBLIC void simsimd_l2sq_u8(simsimd_u8_t const *a,
                                    simsimd_u8_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_I8
...
SIMSIMD_PUBLIC void simsimd_l2sq_f16(simsimd_f16_t const *a,
                                     simsimd_f16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE_F16
...
SIMSIMD_PUBLIC void simsimd_l2sq_bf16(simsimd_bf16_t const *a,
                                      simsimd_bf16_t const *b, simsimd_size_t n,
                                      simsimd_distance_t *d) {
#if SIMSIMD_TARGET_GENOA
...
SIMSIMD_PUBLIC void simsimd_l2sq_f32(simsimd_f32_t const *a,
                                     simsimd_f32_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_l2sq_f64(simsimd_f64_t const *a,
                                     simsimd_f64_t const *b, simsimd_size_t n,
                                     simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_l2_i8(simsimd_i8_t const *a, simsimd_i8_t const *b,
                                  simsimd_size_t n, simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_I8
...
SIMSIMD_PUBLIC void simsimd_l2_u8(simsimd_u8_t const *a, simsimd_u8_t const *b,
                                  simsimd_size_t n, simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_I8
...
SIMSIMD_PUBLIC void simsimd_l2_f16(simsimd_f16_t const *a,
                                   simsimd_f16_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE_F16
...
SIMSIMD_PUBLIC void simsimd_l2_bf16(simsimd_bf16_t const *a,
                                    simsimd_bf16_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
#if SIMSIMD_TARGET_GENOA
...
SIMSIMD_PUBLIC void simsimd_l2_f32(simsimd_f32_t const *a,
                                   simsimd_f32_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_l2_f64(simsimd_f64_t const *a,
                                   simsimd_f64_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
 ...
 *  @note Defined only for binary data.
 */
SIMSIMD_PUBLIC void simsimd_hamming_b8(simsimd_b8_t const *a,
                                       simsimd_b8_t const *b, simsimd_size_t n,
                                       simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
...
SIMSIMD_PUBLIC void simsimd_jaccard_b8(simsimd_b8_t const *a,
                                       simsimd_b8_t const *b, simsimd_size_t n,
                                       simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE
 ...
 *  @note Defined only for floating-point data types.
 */
SIMSIMD_PUBLIC void simsimd_kl_f16(simsimd_f16_t const *a,
                                   simsimd_f16_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_F16
...
SIMSIMD_PUBLIC void simsimd_kl_bf16(simsimd_bf16_t const *a,
                                    simsimd_bf16_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
  simsimd_kl_bf16_serial(a, b, n, d);
}
SIMSIMD_PUBLIC void simsimd_kl_f32(simsimd_f32_t const *a,
                                   simsimd_f32_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON
...
SIMSIMD_PUBLIC void simsimd_kl_f64(simsimd_f64_t const *a,
                                   simsimd_f64_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
  simsimd_kl_f64_serial(a, b, n, d);
}
SIMSIMD_PUBLIC void simsimd_js_f16(simsimd_f16_t const *a,
                                   simsimd_f16_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON_F16
...
SIMSIMD_PUBLIC void simsimd_js_bf16(simsimd_bf16_t const *a,
                                    simsimd_bf16_t const *b, simsimd_size_t n,
                                    simsimd_distance_t *d) {
  simsimd_js_bf16_serial(a, b, n, d);
}
SIMSIMD_PUBLIC void simsimd_js_f32(simsimd_f32_t const *a,
                                   simsimd_f32_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
#if SIMSIMD_TARGET_NEON
...
SIMSIMD_PUBLIC void simsimd_js_f64(simsimd_f64_t const *a,
                                   simsimd_f64_t const *b, simsimd_size_t n,
                                   simsimd_distance_t *d) {
 ...
 */
SIMSIMD_PUBLIC void simsimd_intersect_u16(simsimd_u16_t const *a,
                                          simsimd_u16_t const *b,
...
SIMSIMD_PUBLIC void simsimd_intersect_u32(simsimd_u32_t const *a,
                                          simsimd_u32_t const *b,
                                          simsimd_size_t a_length,
                                          simsimd_size_t b_length,
                                          simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE2
 ...
 *  @param d The output for the number of elements in the intersection.
 */
SIMSIMD_PUBLIC void simsimd_spdot_counts_u16(
...
#if SIMSIMD_TARGET_SVE2
  simsimd_spdot_counts_u16_sve2(a, b, a_weights, b_weights, a_length, b_length,
                                d);
#elif SIMSIMD_TARGET_TURIN
  simsimd_spdot_counts_u16_turin(a, b, a_weights, b_weights, a_length, b_length,
                                 d);
#else
  simsimd_spdot_counts_u16_serial(a, b, a_weights, b_weights, a_length,
                                  b_length, d);
#endif
}

SIMSIMD_PUBLIC void simsimd_spdot_weights_u16(
    simsimd_u16_t const *a, simsimd_u16_t const *b,
    simsimd_bf16_t const *a_weights, simsimd_bf16_t const *b_weights,
    simsimd_size_t a_length, simsimd_size_t b_length, simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SVE2
  simsimd_spdot_weights_u16_sve2(a, b, a_weights, b_weights, a_length, b_length,
                                 d);
#elif SIMSIMD_TARGET_TURIN
  simsimd_spdot_weights_u16_turin(a, b, a_weights, b_weights, a_length,
                                  b_length, d);
#else
  simsimd_spdot_weights_u16_serial(a, b, a_weights, b_weights, a_length,
                                   b_length, d);
#endif
}
 ...
 *  @param d The output for the number of elements in the intersection.
 */
SIMSIMD_PUBLIC void simsimd_bilinear_f64(simsimd_f64_t const *a,
                                         simsimd_f64_t const *b,
                                         simsimd_f64_t const *c,
                                         simsimd_size_t n,
                                         simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SKYLAKE
...
SIMSIMD_PUBLIC void simsimd_bilinear_f32(simsimd_f32_t const *a,
...
SIMSIMD_PUBLIC void simsimd_bilinear_f16(simsimd_f16_t const *a,
                                         simsimd_f16_t const *b,
                                         simsimd_f16_t const *c,
                                         simsimd_size_t n,
                                         simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SAPPHIRE
...
SIMSIMD_PUBLIC void simsimd_bilinear_bf16(simsimd_bf16_t const *a,
                                          simsimd_bf16_t const *b,
                                          simsimd_bf16_t const *c,
                                          simsimd_size_t n,
                                          simsimd_distance_t *d) {
#if SIMSIMD_TARGET_GENOA
...
SIMSIMD_PUBLIC void simsimd_bilinear_f64c(simsimd_f64c_t const *a,
                                          simsimd_f64c_t const *b,
                                          simsimd_f64c_t const *c,
                                          simsimd_size_t n,
                                          simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SKYLAKE
...
SIMSIMD_PUBLIC void simsimd_bilinear_f32c(simsimd_f32c_t const *a,
                                          simsimd_f32c_t const *b,
                                          simsimd_f32c_t const *c,
                                          simsimd_size_t n,
                                          simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SKYLAKE
...
SIMSIMD_PUBLIC void simsimd_bilinear_f16c(simsimd_f16c_t const *a,
                                          simsimd_f16c_t const *b,
                                          simsimd_f16c_t const *c,
                                          simsimd_size_t n,
                                          simsimd_distance_t *d) {
#if SIMSIMD_TARGET_SAPPHIRE
...
SIMSIMD_PUBLIC void simsimd_bilinear_bf16c(simsimd_bf16c_t const *a,
                                           simsimd_bf16c_t const *b,
                                           simsimd_bf16c_t const *c,
                                           simsimd_size_t n,
                                           simsimd_distance_t *d) {
#if SIMSIMD_TARGET_GENOA
...
SIMSIMD_PUBLIC void simsimd_mahalanobis_f64(simsimd_f64_t const *a,
                                            simsimd_f64_t const *b,
                                            simsimd_f64_t const *c,
                                            simsimd_size_t n,
                                            simsimd_distance_t *d) {
...
SIMSIMD_PUBLIC void simsimd_mahalanobis_f32(simsimd_f32_t const *a,
                                            simsimd_f32_t const *b,
                                            simsimd_f32_t const *c,
                                            simsimd_size_t n,
                                            simsimd_distance_t *d) {
...
SIMSIMD_PUBLIC void simsimd_mahalanobis_f16(simsimd_f16_t const *a,
                                            simsimd_f16_t const *b,
                                            simsimd_f16_t const *c,
                                            simsimd_size_t n,
                                            simsimd_distance_t *d) {
...
SIMSIMD_PUBLIC void simsimd_mahalanobis_bf16(simsimd_bf16_t const *a,
                                             simsimd_bf16_t const *b,
                                             simsimd_bf16_t const *c,
                                             simsimd_size_t n,
                                             simsimd_distance_t *d) {
 ...
 */
SIMSIMD_PUBLIC void simsimd_wsum_f64(simsimd_f64_t const *a,
                                     simsimd_f64_t const *b, simsimd_size_t n,
...
SIMSIMD_PUBLIC void simsimd_wsum_f32(simsimd_f32_t const *a,
                                     simsimd_f32_t const *b, simsimd_size_t n,
                                     simsimd_distance_t alpha,
                                     simsimd_distance_t beta,
                                     simsimd_f32_t *r) {
...
SIMSIMD_PUBLIC void simsimd_wsum_bf16(simsimd_bf16_t const *a,
                                      simsimd_bf16_t const *b, simsimd_size_t n,
                                      simsimd_distance_t alpha,
                                      simsimd_distance_t beta,
                                      simsimd_bf16_t *r) {
...
SIMSIMD_PUBLIC void simsimd_wsum_f16(simsimd_f16_t const *a,
                                     simsimd_f16_t const *b, simsimd_size_t n,
                                     simsimd_distance_t alpha,
                                     simsimd_distance_t beta,
                                     simsimd_f16_t *r) {
...
SIMSIMD_PUBLIC void simsimd_wsum_i8(simsimd_i8_t const *a,
                                    simsimd_i8_t const *b, simsimd_size_t n,
                                    simsimd_distance_t alpha,
...
SIMSIMD_PUBLIC void simsimd_wsum_u8(simsimd_u8_t const *a,
                                    simsimd_u8_t const *b, simsimd_size_t n,
                                    simsimd_distance_t alpha,
...
SIMSIMD_PUBLIC void simsimd_fma_f64(simsimd_f64_t const *a,
                                    simsimd_f64_t const *b,
...
SIMSIMD_PUBLIC void simsimd_fma_f32(simsimd_f32_t const *a,
                                    simsimd_f32_t const *b,
...
SIMSIMD_PUBLIC void simsimd_fma_bf16(simsimd_bf16_t const *a,
                                     simsimd_bf16_t const *b,
...
SIMSIMD_PUBLIC void simsimd_fma_f16(simsimd_f16_t const *a,
                                    simsimd_f16_t const *b,
...
SIMSIMD_PUBLIC void simsimd_fma_i8(simsimd_i8_t const *a, simsimd_i8_t const *b,
                                   simsimd_i8_t const *c, simsimd_size_t n,
                                   simsimd_distance_t alpha,
                                   simsimd_distance_t beta, simsimd_i8_t *r) {
#if SIMSIMD_TARGET_SAPPHIRE
...
SIMSIMD_PUBLIC void simsimd_fma_u8(simsimd_u8_t const *a, simsimd_u8_t const *b,
                                   simsimd_u8_t const *c, simsimd_size_t n,
                                   simsimd_distance_t alpha,
                                   simsimd_distance_t beta, simsimd_u8_t *r) {
#if SIMSIMD_TARGET_SAPPHIRE
